...
} //gives you the previous palindrome date
...
function clickHandler(e) {
    var enteredDate = birthDate.value;
    if (enteredDate != "") {
        var listOfDate = enteredDate.split("-");
        ...
        if (isPalindrome) {
            output.innerText = "yup you birthday is a palindrome 😉"
        } else {
            var [ctr, nextDate] = nextPalindromeDate(date);
            var [ctr1, prevDate] = previousPalindromeDate(date);

            if (ctr > ctr1) {
                output.innerText = ` next palindrome date is ${nextDate.day}-${nextDate.month}-${nextDate.year}, you missed it by ${ctr} days! 😔`;

            } else {
                output.innerText = ` next palindrome date is ${prevDate.day}-${prevDate.month}-${prevDate.year}, you missed it by ${ctr1} days! 😔`;
            }
        }
